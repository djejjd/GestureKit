...
- 面板包含可折叠诊断区：最近轻扫成功率、主要失败原因、建议、最近诊断、复制诊断和清空诊断。
...
诊断数据也只保存在本机 `chrome.storage.local`。它只记录手势摘要、动作状态、失败原因、轻扫距离、时长、当前灵敏度和阈值，最多保留最近 100 条；不记录原始触控板帧、网页内容、URL 或浏览历史。
