...
- 三指点按普通网页链接，在当前 Chrome 窗口的当前标签右侧后台打开。
- 三指左滑，切换到当前 Chrome 窗口左侧标签页。
- 三指右滑，切换到当前 Chrome 窗口右侧标签页。
...
- Chrome `chrome.storage.local` 只保存扩展侧状态、缓存和诊断信息。
...
- 在普通 Chrome 网页中，三指点按普通 `<a href>` 链接能后台打开 `http:` 或 `https:` 链接。
- 新标签打开在当前标签右侧，且不切换到新标签。
- 三指左滑能切到同一 Chrome 窗口左侧标签页。
- 三指右滑能切到同一 Chrome 窗口右侧标签页。
- 到达最左或最右标签页时不 wrap，并返回 `edge_reached` 或等价诊断状态。
- 在无链接位置三指点按不会误打开页面。
