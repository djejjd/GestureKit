...
- 如果没有命中链接、没有新鲜 pointer 坐标或当前页面不可注入，则按三指点按的触控板位置作为切 tab fallback：左半区切左 tab，右半区切右 tab。
- 如果 300ms 内连续两次三指点按空白处，则取消单击半区切 tab fallback，改为关闭当前 tab。
...
- 当前标签页不得跳转到被三指点按的链接。
- 不命中链接时，不打开页面；按触控板左/右半区切换到相邻 tab，并沿用边界循环语义。
- 空白处三指双击关闭当前 tab。
- 动作返回成功状态和必要诊断信息。
...
- 手势必须是短促的水平轻扫，建议时长约 60ms-350ms；慢速拖动不触发标签切换。
