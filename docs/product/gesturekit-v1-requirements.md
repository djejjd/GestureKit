...
- 当前标签页是可注入的普通网页。
- Chrome extension 记录到当前活跃标签页内的新鲜 pointer viewport 坐标。
- 坐标命中普通 `<a href>` 链接，或命中位于普通 `<a href>` 内的子元素。
- 链接规范化后的 URL scheme 是 `http:` 或 `https:`。

动作结果：

- 在当前 Chrome 窗口的当前标签右侧创建新标签页。
- 新标签页 `active: false`，不抢占当前标签。
- 动作返回成功状态和必要诊断信息。
...
- 已经位于最左侧标签页时，不执行切换，返回 `edge_reached` 或等价诊断状态。
...
- 已经位于最右侧标签页时，不执行切换，返回 `edge_reached` 或等价诊断状态。
...
- 至少一次手动端到端验证覆盖三指点按链接、三指左滑、三指右滑。
- 失败场景至少覆盖无链接、无相邻 tab、页面不可注入、native host 断开。
