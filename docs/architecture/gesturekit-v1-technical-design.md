...
GestureKit 是一个 macOS 自用优先、后续可开源的触控板手势增强工具。V1 聚焦 Chrome，通过三指点按和三指滑动替代高频浏览器操作。
...
V1 不允许 Chrome 扩展单独编辑规则，避免双写和同步冲突。
...
- 到达第一个 tab 时不 wrap，返回 `edge_reached`。
...
- 在当前 Chrome 窗口内查找 active tab。
- 激活同一窗口内 `index + 1` 的 tab。
- 到达最后一个 tab 时不 wrap，返回 `edge_reached`。
