...
> 2026-07-10 架构演进说明：可靠性证据链、App 主窗口、统一配置主存储、手势组合和通用 `ActionProvider` 边界以 `docs/architecture/可靠性可观测平台-架构.md` 为准。本文保留 V1 Chrome 实现基线与既有通信、坐标和安全决策。
...
本节记录当前 Chrome V1 实现的 legacy `GestureKitMessage version: 1`，用于迁移和兼容测试，不再是目标 Provider 公共协议。目标协议为 `docs/architecture/可靠性可观测平台-架构.md` 定义的 Provider Protocol v2。
...
正式开发前的触控板输入稳定性以 `docs/research/触控板手势稳定性矩阵.md` 为准。产品动作端到端验收在正式产品实现阶段单独执行。
