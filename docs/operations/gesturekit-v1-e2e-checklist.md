...
- [ ] 点击 Chrome 工具栏 GestureKit 图标，popup 能显示安全模式、高效模式、三类开关、轻扫灵敏度、三个手感参数、连接状态和灵敏度同步状态。
...
- [ ] popup 显示“轻扫表现”：最近轻扫成功率、主要失败原因和建议。
- [ ] 点击“展开”后能看到最近诊断，轻扫失败项包含原因、dx、dy、duration 和灵敏度。
- [ ] 点击“复制诊断”后，剪贴板中包含 `GestureKit Diagnostics`，且不包含 URL 或网页内容。
