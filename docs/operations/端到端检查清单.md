...
- [ ] 已按 `docs/operations/本地安装.md` 运行 `./scripts/dev/install-local.sh` 并加载 unpacked extension，无需记录或复制扩展 ID。
...
- [ ] 点击”打开安装说明”，确认能打开仓库 `docs/operations/本地安装.md`（需设置 `GESTUREKIT_REPO_ROOT` 环境变量或从仓库目录启动）。
- [ ] 点击”打开排障文档”，确认能打开仓库 `docs/operations/排障手册.md`。
...
- 缺口 I2（2026-08-08 Task 4 部分关闭）：leaseExpiry 已补 guard trace 断言——时间窗口内 guard
  ...
  `click_blocked`/`lease_expired`（点击未被拦截），详见 `docs/operations/排障手册.md`
...
`docs/operations/排障手册.md` 的“链接可靠性 E2E”章节。
...
如果 smoke 页面或 native host 链路异常，先看 `docs/operations/排障手册.md`。
