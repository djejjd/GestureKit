...
describe("ChromeProvider", () => {
  ...
  it("executes browser.tab.activate_next without a link targetRef", async () => {
    ...
    await expect(provider.execute({ operationId: "op-2", actionId: "browser.tab.activate_next", contextId: "unused", gestureSessionId: "g-1", deadline: Date.now() + 1_000 })).resolves.toMatchObject({ status: "success" });
