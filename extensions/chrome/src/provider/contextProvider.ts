...
export class ContextProvider {
  private readonly targets = new Map<string, { url: string; expiresAt: number }>();
  ...
  snapshot(resolvedURL: string | null, now = Date.now(), ttlMs = 1_500): ContextSnapshotPayload {
    ...
    this.targets.set(targetRef, { url: resolvedURL, expiresAt });
    return { contextId, pageIdentity: url.toString(), expiresAt, targetKind: "standard_link", targetRef };
  }

  resolveTarget(targetRef: string, now = Date.now()): string | null {
    const target = this.targets.get(targetRef);
    if (!target || target.expiresAt < now) { this.targets.delete(targetRef); return null; }
    return target.url;
  }
